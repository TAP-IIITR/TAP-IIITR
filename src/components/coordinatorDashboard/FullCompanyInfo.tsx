...
import { Trash2, Download, ArrowLeft, Calendar, Briefcase, MapPin, Clock, ChevronRight, Eye, IndianRupee } from "lucide-react";
...
const FullCompanyInfo = () => {
  ...
  return (
    <div className="flex flex-col gap-6 p-4 md:p-6 bg-gray-50 min-h-screen">
      ...
      >
        <ArrowLeft size={18} className="transition-transform duration-300 group-hover:scale-110" /> 
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="w-full lg:w-3/4 flex flex-col gap-6">
          <div
            className="bg-white rounded-xl p-6 transition-all duration-300 hover:shadow-xl shadow-md border border-gray-100"
          >
            ...
            <div className="flex flex-wrap gap-5 mb-6">
              <div className="flex items-center gap-2 group">
                <div className="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center transition-all duration-300 group-hover:bg-indigo-100">
                  <MapPin size={16} className="text-indigo-700 transition-transform duration-300 group-hover:scale-110" />
                </div>
                <p className="text-gray-700 font-medium text-sm">{jobData.location}</p>
              </div>
              
              <div className="flex items-center gap-2 group">
                <div className="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center transition-all duration-300 group-hover:bg-indigo-100">
                  <Briefcase size={16} className="text-indigo-700 transition-transform duration-300 group-hover:scale-110" />
                </div>
                <p className="text-gray-700 font-medium text-sm">{jobData.jobType}</p>
              </div>
              
              <div className="flex items-center gap-2 group">
                <div className="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center transition-all duration-300 group-hover:bg-indigo-100">
                  <IndianRupee size={16} className="text-indigo-700 transition-transform duration-300 group-hover:scale-110" />
                </div>
                <p className="text-gray-700 font-medium text-sm">{jobData.package}</p>
              </div>
            </div>
        ...
        <div className="w-full lg:w-1/4 flex flex-col gap-6 order-first lg:order-last">
          {/* Important Dates */}
          <div
            className="bg-white rounded-xl p-6 border border-gray-100 transition-all duration-300 hover:shadow-xl shadow-md"
          >
            ...
            <div className="space-y-4">
              <div className="group flex items-center gap-3 transition-all duration-300 hover:translate-x-1">
                <div className="flex-shrink-0 w-10 h-10 rounded-full bg-indigo-50 flex items-center justify-center transition-all duration-300 group-hover:bg-indigo-100">
                  <Calendar size={18} className="text-indigo-700 transition-transform duration-300 group-hover:scale-110" />
                </div>
                <div>
      ...
      <div
        className="w-full bg-white rounded-xl p-6 border border-gray-100 transition-all duration-300 hover:shadow-xl shadow-md mt-4"
      >
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-bold text-gray-800">Student Applications</h3>
          <button
        ...
        <div className="flex flex-col gap-4">
          {applications && applications.length > 0 ? (
              ...
              >
                <div className="mb-4 md:mb-0">
                  {Object.entries(application?.form).map(([key, value]) => (
                    ...
                    >
                      <span className="font-medium text-gray-800">{key.replace(/([A-Z])/g, " $1")}:</span> {String(value) || "N/A"}
                    </p>
                  ))}
                </div>
            ...
            <div className="text-center py-12 bg-gray-50 rounded-xl border border-dashed border-gray-300">
              ...
              <p className="text-gray-500 font-medium">No applications found for this job posting</p>
              <p className="text-sm text-gray-400 mt-1">Applications will appear here once students apply</p>
            </div>
          )}
        </div>
      </div>
